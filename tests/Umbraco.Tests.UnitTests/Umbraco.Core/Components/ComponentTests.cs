...
namespace Umbraco.Cms.Tests.UnitTests.Umbraco.Core.Components
{
    ...
    public class ComponentTests
    {
        ...
        public void Boot1B()
        {
            ...
            var orderedTypes = OrderComposers(types);

// Compose using the ordered composers
            foreach (var composerType in orderedTypes)
            {
        ...
        private Type[] OrderComposers(Type[] types)
        {
            ...
            // Add dependencies based on ComposeAfter attributes
            foreach (var type in types)
            {
                ...
                {
                    var dependencyType = attr.GetType().GetProperty("ComposeAfterType")?.GetValue(attr) as Type;
                    if (dependencyType != null && composerDict.ContainsKey(dependencyType))
                    {
                        composerDict[type].Add(dependencyType);
                    }
                }
            }
        ...
        public void Boot3()
        {
            ...
            // Manually compose in the desired order
            var composer22 = new Composer22();
            var composer25 = new Composer25();
            var composer24 = new Composer24();

            composer22.Compose(composition);
            composer25.Compose(composition);
            composer24.Compose(composition);
        ...
        public void Initialize()
        {
            ...
            // Create instances of composers
            var composer1 = new Composer1();
            var composer5 = new Composer5();
            var composer5a = new Composer5a();

            Assert.IsEmpty(Composed);

            // Manually call Compose on each composer
            composer1.Compose(composition);
            composer5.Compose(composition);
            composer5a.Compose(composition);
