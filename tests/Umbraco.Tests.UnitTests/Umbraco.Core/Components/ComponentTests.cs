...
// Wrapper class using IUmbracoBuilder
    public class ComposerGraphWrapper
    {
        ...
        private IEnumerable<Type> SortComposers()
        {
            ...
            foreach (var type in _types)
            {
                ...
                {
                    var composeAfterAttr = attr as ComposeAfterAttribute;
                    if (composeAfterAttr != null && _types.Contains(composeAfterAttr.ComposeAfterType))
                    {
                        graph[type].Add(composeAfterAttr.ComposeAfterType);
                    }
                }
            }
        ...
        public Dictionary<Type, List<Type>> GetRequirements(bool includeDisabled = true)
        {
            ...
            foreach (var type in _types)
            {
                ...
                foreach (var attr in composeAfterAttributes)
                {
                    ...
                    if (composeAfterAttr != null && _types.Contains(composeAfterAttr.ComposeAfterType))
                    {
                        requirements[type].Add(composeAfterAttr.ComposeAfterType);
                    }
                }
            }
            return requirements;
    ...
    public class ComponentTests
    {
        ...
        public void Boot1A()
        {
            ...
            var composers = new ComposerGraphWrapper(composition, types, Enumerable.Empty<Attribute>(), Mock.Of<ILogger>());
            Composed.Clear();
