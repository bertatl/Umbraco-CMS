...
public static class EmailMessageExtensions
{
    ...
    {
        var from = new NotificationEmailAddress(emailMessage.From ?? configuredSender);
        var to = emailMessage.To.Select(address => new NotificationEmailAddress(address)).ToList();
        var cc = emailMessage.Cc?.Select(address => new NotificationEmailAddress(address)).ToList() ?? new List<NotificationEmailAddress>();
        var bcc = emailMessage.Bcc?.Select(address => new NotificationEmailAddress(address)).ToList() ?? new List<NotificationEmailAddress>();
        var replyTo = emailMessage.ReplyTo?.Select(address => new NotificationEmailAddress(address)).ToList() ?? new List<NotificationEmailAddress>();
