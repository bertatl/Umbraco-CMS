...
public static class EmailMessageExtensions
{
    ...
    {
        var from = new NotificationEmailAddress(string.Empty, emailMessage.From ?? configuredSender);
        var to = emailMessage.To.Select(address => new NotificationEmailAddress(string.Empty, address)).ToList();
        var cc = emailMessage.Cc?.Select(address => new NotificationEmailAddress(string.Empty, address)).ToList() ?? new List<NotificationEmailAddress>();
        var bcc = emailMessage.Bcc?.Select(address => new NotificationEmailAddress(string.Empty, address)).ToList() ?? new List<NotificationEmailAddress>();
        var replyTo = emailMessage.ReplyTo?.Select(address => new NotificationEmailAddress(string.Empty, address)).ToList() ?? new List<NotificationEmailAddress>();
