...
namespace Umbraco.Cms.Tests.UnitTests.Umbraco.Infrastructure.Media
{
    ...
    public class ImageSharpImageUrlGeneratorTests
    {
        ...
        public ImageSharpImageUrlGeneratorTests()
        {
            var options = new Mock<IOptions<ImageSharpConfiguration>>();
            options.Setup(x => x.Value).Returns(new ImageSharpConfiguration());
            _generator = new ImageSharpImageUrlGenerator(options.Object);
        }
