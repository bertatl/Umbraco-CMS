...
namespace Umbraco.Cms.Tests.UnitTests.Umbraco.Web.BackOffice.Extensions
{
    ...
    public class ModelStateExtensionsTests
    {
        ...
        public void Get_Cultures_With_Errors()
        {
            ...
            IReadOnlyList<(string culture, string segment)> propertyResult = ms.GetVariantsWithPropertyErrors("en-US");
        ...
        public void Get_Cultures_With_Property_Errors()
        {
            ...
            var result = ms.Where(kvp => kvp.Value.Errors.Count > 0)
                           .Select(kvp => kvp.Key)
                           .Where(key => key.StartsWith("_Properties."))
                           .Select(key =>
                           {
                               var parts = key.Split('.');
                               return parts.Length >= 3 ? (parts[2], parts.Length > 3 ? parts[3] : null) : ("invariant", null);
                           })
                           .Distinct()
                           .ToList();
            ...
            Assert.IsTrue(result.Any(r => r.Item1 == "en-US"));
            Assert.IsTrue(result.Any(r => r.Item1 == "invariant"));
        }
